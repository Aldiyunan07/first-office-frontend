    ...
    return (
        <>
            ...
            <div id="Banner" className="relative w-full h-[240px] flex items-center shrink-0 overflow-hidden -mb-[50px]">
                ...
                <img src={bookingDetails ? `${baseURL}/${bookingDetails.officeSpace.thumbnail}` : '/assets/images/thumbnails/thumbnail-details-5.png'} className="absolute w-full h-full object-cover object-top" alt="" />
            </div>
                ...
                <form onSubmit={handleSubmit} className="flex items-end rounded-[20px] border border-[#E0DEF7] p-[30px] gap-[16px] bg-white">
                    <div className="flex flex-col w-full gap-2">
                        ...
                        <div className="flex items-center rounded-full border border-[#000929] px-5 gap-[10px] transition-all duration-300 focus-within:ring-2 focus-within:ring-[#0D903A]">
                            ...
                            <input type="text" onChange={handleChange} name="booking_trx_id" value={formData.booking_trx_id} id="booking_trx_id" className="appearance-none outline-none w-full py-3 font-semibold placeholder:font-normal placeholder:text-[#000929]" placeholder="Write your booking trx id" />
                        </div>
                        {formErrors.find((error) => error.path.includes('booking_trx_id')) && <p className="text-red-500">Booking TRX ID is required</p>}
                    </div>
                    <div className="flex flex-col w-full gap-2">
                        ...
                        <div className="flex items-center rounded-full border border-[#000929] px-5 gap-[10px] transition-all duration-300 focus-within:ring-2 focus-within:ring-[#0D903A]">
                            ...
                            <input type="tel" onChange={handleChange} value={formData.phone_number} name="phone_number" id="phone_number" className="appearance-none outline-none w-full py-3 font-semibold placeholder:font-normal placeholder:text-[#000929]" placeholder="Write your valid number" />
                        </div>
                        {formErrors.find((error) => error.path.includes('phone_number')) && <p className="text-red-500">Phone number is required</p>}
                    </div>
                    <button disabled={isLoading} className="flex items-center justify-center rounded-full p-[12px_30px] gap-3 bg-[#0D903A] font-bold text-[#F7F7FD]">
                        <span className="text-nowrap">{isLoading ? 'Is Loading ...' : 'Check Booking'}</span>
                    </button>
                </form>
                {bookingDetails && (
                        ...
                        <div className="flex flex-col h-fit shrink-0 rounded-[20px] border border-[#E0DEF7] p-[30px] gap-[30px] bg-white">
                            <div className="flex items-center gap-4">
                                <div className="flex shrink-0 w-[140px] h-[100px] rounded-[20px] overflow-hidden">
                                    <img src={`${baseURL}/${bookingDetails.officeSpace.thumbnail}`} className="w-full h-full object-cover" alt="thumbnail" />
                                </div>
                                <div className="flex flex-col gap-2">
                                    <p className="font-bold text-xl leading-[30px]">{bookingDetails.officeSpace.name}</p>
                            ...
                            <div className="flex flex-col gap-5">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <p className="font-bold text-[22px] leading-[33px] text-[#0D903A]">
                                        Rp{' '}
                                        {bookingDetails.total_amount.toLocaleString('id', {
